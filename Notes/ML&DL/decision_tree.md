...
注意，$C(T)$是**全局**的一个损失函数，当一棵树在节点t处被剪枝时，误差增加了$C(t)-C(T_t)$；但剪枝后叶子数减少了$|T_t|-1$个，复杂性减少了。因此考虑到数的复杂性因素，树在节点t处被剪枝后的误差增加率为$\alpha=\frac{C(t)-C(T_t)}{|T_t|-1}$. [7]

**为什么选择最小的$g(t)$为$\alpha_i​$**

这个$g(t)​$是剪枝的阈值。对某节点t，当$\alpha=g(t)​$时，剪和不剪对总体损失函数一样。如果$\alpha>g(t)​$，不剪的整体损失函数更大，应该剪；反之，$\alpha​$小于$g(t)​$就不该剪。损失函数可以对整个树，也可以是以t为根的子树（对t剪枝前后两者的增减是相同的）。

对同一颗树，$\alpha$从0开始增大，有某**一颗树**该剪，其他树不该剪，即$\alpha$超过了某个节点的$g(t)$但没有超过其他节点的$g(t)$。随着$\alpha$逐渐变大，不断的剪枝，得到子树序列 [8]。关键前面提到了：序列的子树是嵌套的！

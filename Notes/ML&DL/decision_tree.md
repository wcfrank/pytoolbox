...
注意，$C_{\alpha}(T)​$是**全局**的一个损失函数，当一棵树在节点t处被剪枝时，误差增加了$C(t)-C(T_t)​$；但剪枝后叶子数减少了$|T_t|-1​$个，复杂性减少了。因此考虑到数的复杂性因素，树在节点t处被剪枝后的误差增加率为$\alpha=\frac{C(t)-C(T_t)}{|T_t|-1}​$. [7]
...
这个$g(t)$是剪枝的阈值。对同一颗树，$\alpha$从0开始增大，有某**一颗树**该剪，其他树不该剪，即$\alpha$达到了某个节点的$g(t)$但没有满足其他节点的$g(t)$。假设有多个内部节点$g(t_1)<g(t_2)<\dots$，如果选择剪枝$t_2$，$\alpha=g(t_2)$，这时剪不剪$t_2$总体损失函数相同，但是剪$t_1$会使损失函数更小，所以剪掉$t_1$的子树也是一个剪枝方案，不要在剪$t_2$的子树之前遗漏$t_1$的子树。

随着$\alpha​$逐渐变大，不断的剪枝，得到子树序列 [8]。关键前面提到了：序列的子树是嵌套的！
